...
# print(gaf[gaf["GO_ID"] == "GO:0009536"])
...
cosine_matrix_rounded = cosine_similarity((X > 0).astype(float)).round(4)
cosine_matrix = cosine_similarity((X > 0).astype(float))
...
cosine_df = pd.DataFrame(cosine_matrix, index=d[0], columns=d[0])
cosine_df_rounded = pd.DataFrame(cosine_matrix_rounded, index=d[0], columns=d[0])
print(cosine_df_rounded)
...
cosine_df.to_csv(RESULTS + "gene_cosine_matrix.csv", mode="w")
cosine_df_rounded.to_csv(RESULTS + "gene_cosine_matrix_rounded.csv", mode="w")
...
plt.title("cosine GO Annotation Heatmap")
fig.savefig(FIGURES + "cosine_go_ann_heatmap.png", dpi=600, bbox_inches="tight")
